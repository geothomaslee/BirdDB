...
def plot_sankey_tree(db, mode: str='species',tree_style=1,_debug: bool=True):
    ...
    fig = _plotSankey(pddf,unique_df,mode=tree_style)
...
def _plotSankey(df,full_df,mode: int=1):
    """Wrapper function for which style of Sankey to return"""
    if mode==1:
        return _plotSankeySimple(df)
    elif mode==2:
        return _plotSankeyComplex(df,full_df)
    else:
...
def _plotSankeyComplex(df, full_df, node_spacing: float=0.015):
    # Get unique labels from both source and target columns
    labels = list(pd.unique(df[['source', 'target']].values.ravel('K')))
